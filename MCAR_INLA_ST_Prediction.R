...
######################################################################################
MCAR_INLA_ST_Prediction <- function(carto=NULL, data=NULL, ID.area=NULL, ID.year=NULL, ID.disease=NULL,
  ...
  Model.with.prediction <- inla(formula.with.factors, family="poisson", data=data.INLA, E=E,
  ...
  Model.with.prediction$Mmodel <- list(spatial=spatial, temporal=temporal, interaction=interaction)
  #Model <- list(cluster_model = Model.cluster, association_model = Model.with.factor)
  
  return(Model.with.prediction)
}
...
Mmodel_compute_cor <- function(model, n.sample=1000){
